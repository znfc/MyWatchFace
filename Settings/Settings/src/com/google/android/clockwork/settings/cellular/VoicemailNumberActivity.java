...
/**
 * Activity to allow user to set voicemail number manually.
 */
public class VoicemailNumberActivity extends WearableActivity implements DialpadHost {
    @Override
    ...
    private void verifyPhoneNumberChanged(final String newNumber, final String oldNumber) {
        final AcceptDenyDialog d = new AcceptDenyDialog(VoicemailNumberActivity.this);
        d.setTitle(R.string.pref_voicemailNumber);
        d.setMessage(getApplicationContext().getString(R.string.settings_update_phone_number,
                SetNumberService.formatNumber(VoicemailNumberActivity.this, oldNumber),
                SetNumberService.formatNumber(VoicemailNumberActivity.this, newNumber)));
        d.setPositiveButton((dialog, which) -> {
            dialog.dismiss();
            Parcelable p = getIntent().getParcelableExtra(SetNumberService.EXTRA_RESULT_RECEIVER);
            Intent intent = new Intent(SetNumberService.ACTION_SET_VOICEMAIL_NUMBER)
