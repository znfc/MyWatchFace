 ...
 */
public class InputMethodPreference extends SwitchPreference implements
    ...
    private EnforcedAdmin mDisabledByAdmin;
    ...
    public void setDisabledByAdmin(EnforcedAdmin admin) {
        mDisabledByAdmin = admin;
        setEnabled(mDisabledByAdmin != null);
    }

    @Override
    protected void onClick() {
        if (isDisabledByAdmin()) {
            RestrictedLockUtils.sendShowAdminSupportDetailsIntent(getContext(), mDisabledByAdmin);
            return;
        }
        super.onClick();
    }
    ...
    public boolean onPreferenceChange(final Preference preference, final Object newValue) {
        ...
        if (InputMethodUtils.isSystemIme(mImi)) {
            ...
            // but we might need to prompt if it's not Direct Boot aware and a keyguard is enabled.
            if (!mImi.getServiceInfo().directBootAware && mKeyguardManager.isDeviceSecure()) {
    ...
    private void showSecurityWarnDialog() {
        ...
        builder.setPositiveButton(android.R.string.ok, (dialog, which) -> {
            ...
            // need to prompt if it's not Direct Boot aware and a keyguard is enabled.
            if (!mImi.getServiceInfo().directBootAware && mKeyguardManager.isDeviceSecure()) {
                showDirectBootWarnDialog();
            } else {
                setCheckedInternal(true);
    ...
    public int compareTo(final InputMethodPreference rhs, final Collator collator) {
        ...
        if (mHasPriorityInSorting == rhs.mHasPriorityInSorting) {
            ...
            final boolean emptyT0 = TextUtils.isEmpty(t0);
            final boolean emptyT1 = TextUtils.isEmpty(t1);
            if (!emptyT0 && !emptyT1) {
                return collator.compare(t0.toString(), t1.toString());
            }
            // For historical reasons, an empty text needs to be first.
            return (emptyT0 ? -1 : 0) - (emptyT1 ? -1 : 0);
        }
